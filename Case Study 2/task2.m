...
wind_array = 2065:5:5000;%   4130/2---5000/2 (MW)
...
vNI(1) = 0;                  % Spilling volume of NI lake (m^3)
vSI(1) = 0;                  % Spilling volume of SI lake (m^3)
...
for m = 1:length(wind_array)
    ...
    for n=2:tarray(end)
        ...
        vNI(n) = vNI(n-1) + (Spill_NI_dvdt*3600);%Current lake volume of NI lake (m^3)
        vSI(n) = vSI(n-1) + (Spill_SI_dvdt*3600);%Current lake volume of SI lake (m^3)
        % Cost of spilled water in NI&SI ($)
        cost_NI(n) = 90*density*(vNI(n)-vNI(n-1))*g*(hNI(n)-h_ge_NI)/(3600*1e6); 
        cost_SI(n) = 90*density*(vSI(n)-vSI(n-1))*g*(hSI(n)-h_ge_SI)/(3600*1e6); 
    end
    %---------Checking--------------
    for ii=2:tarray(end)
